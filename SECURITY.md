...
<https://github.com/voicetel/python-sdk/security/advisories/new>).
...
- The SDK pins TLS 1.2+ via `httpx` defaults and reuses connections
  across requests. If you supply a custom `httpx.Client` /
  `AsyncClient`, you are responsible for matching that posture.
...
Out of scope: vulnerabilities in the `voicetel` PyPI package caused
